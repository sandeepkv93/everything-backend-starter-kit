...

{
  ...
  activeRequests: 'sum(http_server_active_requests{job="%s"})' % c.job,

  requestClass2xx: 'sum(rate(http_server_request_duration_seconds_count{job="%s",http_response_status_code=~"2.."}[5m]))' % c.job,
  requestClass4xx: 'sum(rate(http_server_request_duration_seconds_count{job="%s",http_response_status_code=~"4.."}[5m]))' % c.job,
  requestClass5xx: 'sum(rate(http_server_request_duration_seconds_count{job="%s",http_response_status_code=~"5.."}[5m]))' % c.job,

  authLoginAttempts: 'sum(rate(auth_login_attempts_total{job="%s"}[5m])) by (status)' % c.job,
  authRefreshAttempts: 'sum(rate(auth_refresh_attempts_total{job="%s"}[5m])) by (status)' % c.job,
  rateLimitDecisions: 'sum(rate(http_rate_limit_decisions_total{job="%s"}[5m])) by (outcome)' % c.job,
  repositoryErrors: 'sum(rate(repository_operations_total{job="%s",outcome="error"}[5m])) by (repo)' % c.job,
  unhealthyChecks: 'sum(rate(health_check_results_total{job="%s",outcome="unhealthy"}[5m])) by (check)' % c.job,
